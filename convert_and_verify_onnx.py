...
if conv_int_node:
    ...
    # Remove orphan nodes connected to pixel_values
    for node in list(graph.node):
        if "pixel_values" in node.input and node.op_type in ["DynamicQuantizeLinear", "QuantizeLinear", "Cast"]:
            graph.node.remove(node)
            print(f"Removed node: {node.name}")

    # Remove any unreferenced initializers or inputs
    all_inputs = set()
    for n in graph.node:
        for inp in n.input:
            all_inputs.add(inp)
            
    graph.initializer[:] = [init for init in graph.initializer if init.name in all_inputs]

    print("Check model validity with onnx.checker...")
    try:
        onnx.checker.check_model(model)
        print("ONNX model check PASSED!")
    except Exception as e:
        print("ONNX model check WARNING:", e)

onnx.save(model, output_path)
print(f"Saved verified ONNX model to {output_path}")
